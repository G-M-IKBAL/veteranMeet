...
function Profile({email}){
    ...
    async function handle(){
        ...
        const checkAcc = email.split('@');
        setCheckAccount(checkAcc);
        if(checkAcc[1]!=="org.com")
        {
            ...
            await fetch(`http://localhost:3000/api/about/organization/${email}`)
            .then(res => res.json())
            .then((data) => {
                if(data) {
                ...
                <div className="flex flex-col">
                    <h4 className="text-lg font-semibold text-center md:text-left">About</h4>
                    {
                        ...
                        (
                            ...
                            :
                            <div>
                                <p className="dark:text-gray-400">Name:{info.name_of_org}</p>
                                <p className="dark:text-gray-400">Category:{info.category}</p>
                                <p className="dark:text-gray-400">City:{info.city}</p>
                                <p className="dark:text-gray-400">contact:{info.contact}</p>
